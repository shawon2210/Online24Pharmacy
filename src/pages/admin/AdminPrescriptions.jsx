...
export default function AdminPrescriptions() {
  ...
  const updateStatus = async (id, status, adminNotes) => {
    try {
      ...
      await axios.put(`${API_URL}/api/admin/prescriptions/${id}/review`, 
        { status, adminNotes },
        { headers: { Authorization: `Bearer ${token}` }}
      );
        ...
        <table className="min-w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {prescriptions.length === 0 ? (
              <tr>
                <td colSpan="5" className="px-6 py-8 text-center text-gray-500">No prescriptions found</td>
              </tr>
            ) : (
              prescriptions.map((rx) => (
                <tr key={rx.id}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {rx.user && <div className="text-xs text-gray-500">{rx.user.email}</div>}
                  </td>
