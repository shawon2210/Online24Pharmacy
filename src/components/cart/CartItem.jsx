...
export default function CartItem({ item, onUpdateQuantity, onRemove }) {
  ...
  return (
    <div className="bg-background dark:bg-card rounded-lg sm:rounded-xl shadow-md dark:shadow-lg border border-border p-3 sm:p-4 lg:p-6 hover:shadow-lg dark:hover:shadow-xl transition-all duration-300 flex flex-col sm:flex-row gap-3 sm:gap-4 lg:gap-6 items-start">
      {/* Product Image */}
      ...
      {/* Quantity Control */}
      <div className="flex items-center gap-1 sm:gap-2 bg-muted rounded-lg p-1.5 sm:p-2 lg:p-3 shrink-0">
        <button
          onClick={handleDecrease}
          className="w-7 h-7 sm:w-8 sm:h-8 lg:w-9 lg:h-9 flex items-center justify-center rounded-lg hover:bg-border text-foreground font-bold transition-all text-sm sm:text-base"
        >
          −
        </button>
        <span className="px-2 sm:px-3 lg:px-4 font-bold text-foreground dark:text-foreground min-w-8 text-center text-xs sm:text-sm lg:text-base">
          {item.quantity}
        </span>
        <button
