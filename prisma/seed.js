...
async function main() {
  ...
  const feverCare = await prisma.category.upsert({
    where: { slug: 'fever-care' },
    update: {},
    create: {
      name: 'Fever Care',
      slug: 'fever-care',
      description: 'Products for fever relief and care.',
    },
  });
  ...
  await prisma.product.upsert({
    where: { sku: 'ACET-CODEINE-30' },
    update: {},
    create: {
      name: 'Acetaminophen + Codeine',
      slug: 'acetaminophen-+-codeine',
      sku: 'ACET-CODEINE-30',
      description: 'Combination medication for moderate to severe pain relief.',
      price: 18.99,
      stockQuantity: 150,
      categoryId: painRelief.id,
      subcategoryId: otcPainRelief.id,
  ...
  await prisma.product.upsert({
    where: { sku: 'GREENPAS-100' },
    update: {},
    create: {
      name: 'Greenpas',
      slug: 'greenpas',
      sku: 'GREENPAS-100',
      description: 'Herbal supplement for general wellness.',
      price: 25.0,
      stockQuantity: 300,
      categoryId: vitamins.id,
      subcategoryId: generalHealth.id,
  ...
  // Additional products for fever care
  await prisma.product.upsert({
    where: { sku: 'PED-IBU-SYRUP-100ML' },
    update: {
      name: 'Pediatric Ibuprofen Syrup (100ml)',
      slug: 'pediatric-ibuprofen-syrup-100ml',
      description: 'Ibuprofen syrup for children to reduce fever and pain.',
      price: 22.0,
      stockQuantity: 150,
      categoryId: feverCare.id,
      subcategoryId: otcPainRelief.id,
      brand: 'Generic',
      isOTC: true,
      requiresPrescription: false,
  ...
  await prisma.product.upsert({
    where: { sku: 'PARA-1G-SUPP' },
    update: {
      name: 'Paracetamol 1g Suppository',
      slug: 'paracetamol-1g-suppository',
      description: 'Paracetamol suppository for fever relief.',
      price: 30.0,
      stockQuantity: 100,
      categoryId: feverCare.id,
      subcategoryId: otcPainRelief.id,
  ...
  await prisma.product.upsert({
    where: { sku: 'NAPROX-SOD-220MG' },
    update: {
      name: 'Naproxen Sodium 220mg',
      slug: 'naproxen-sodium-220mg',
      description: 'Naproxen sodium for pain and fever relief.',
      price: 80.0,
      stockQuantity: 200,
      categoryId: feverCare.id,
      subcategoryId: otcPainRelief.id,
  ...
  await prisma.product.upsert({
    where: { sku: 'NAPROX-250MG' },
    update: {
      name: 'Naproxen 250mg',
      slug: 'naproxen-250mg',
      description: 'Naproxen tablets for pain and inflammation.',
      price: 20.0,
      stockQuantity: 300,
      categoryId: feverCare.id,
      subcategoryId: otcPainRelief.id,
  ...
  await prisma.product.upsert({
    where: { sku: 'PARA-SYRUP-120-100ML' },
    update: {
      name: 'Paracetamol Syrup 120mg/5ml (100ml)',
      slug: 'paracetamol-syrup-120mg-5ml-100ml',
  ...
  await prisma.product.upsert({
    where: { sku: 'PARA-650MG' },
    update: {
      name: 'Paracetamol 650mg',
      slug: 'paracetamol-650mg',
      description: 'Paracetamol tablets for fever and pain.',
      price: 20.0,
      stockQuantity: 400,
      categoryId: feverCare.id,
      subcategoryId: otcPainRelief.id,
  ...
  await prisma.product.upsert({
    where: { sku: 'NAPA-EXTRA' },
    update: {
      name: 'NapaExtra',
      slug: 'napaextra',
      description: 'Extra strength paracetamol for severe pain and fever.',
      price: 12.0,
      stockQuantity: 500,
      categoryId: feverCare.id,
      subcategoryId: otcPainRelief.id,
